...
<html lang="es">
    <head>
        <meta charset="uft-8" />
        <title>Sistema de Validación de Cuentas</title>
    ...
    <body>
        ...
        <div id="main_container">
            ...
            <div class="brand">
                <h1><span>Sistema de Validación de Cuentas</span></h1>
            </div>
            ...
            <div class="container">
                ...
                <div class="login-content">
                    ...
                    <!-- <form name="form1" id="form1" method="post" enctype="application/x-www-form-urlencoded"> -->
                        ...
                        <div>
                            <label class="label"><?//=$_SESSION['ValCuentasError']?></label>
                        </div>
            ...
            <div id="errors">
                <?php //echo $_SESSION['ValCuentasFError']; ?>
            </div>
        </div>
        <div id="footer">
            <?php include("P_Cuentas00_footer_nuevo.php"); ?>
        </div>
<!--    El filemtime asegura que se vuelva a cargar el JS solo si este se modifico, evitando tener que borrar cookies -->
        ...
        <script src="jsP/P_encripta_.js?v=<?php echo filemtime('jsP/P_encripta_.js'); ?>"></script>
        <script src="jsP/P_login_.js?v=<?php echo filemtime('jsP/P_login_.js'); ?>"></script>
        <script>
            function onClick(e) {
                e.preventDefault();
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                formulario.addEventListener('submit', async function(event) {
                    ...
                    const vUser   = document.getElementById('user_login').value.trim();
                    const vClave  = document.getElementById('password_login').value.trim();
                    ...
                    if (vUser === '' || vClave === "" ) {
                        ...
                        try {
                            const oPass       = document.getElementById('password_login')
                            const passInput   = oPass.value;
                            const cEncriptado = await encriptaNativo(passInput);
                            oPass.value       = cEncriptado
                            //console.log("Dato original (A):", passInput);
                            //console.log("Dato encriptado (cEncriptado):", cEncriptado);
                            // Este cEncriptado se enviaría a PHP
                            validaCredenciales(vUser,cEncriptado)
                        } catch (e) {
                            console.error("Error de encriptación:", e);
                        }
                    }
                });
            });
            ...
            function verificarError() {
                ...
                var error = "<?php 
                    if ( isset($_SESSION['PCuentasError']) ){
                        if (trim($_SESSION['PCuentasError'])!=""){
                            echo $_SESSION['PCUentasError'];
                        }else{
                            echo "";
                        }
                    }else{
            ...
            // Verificar si error tiene algún valor
                ...
                // Llamar a la función mandaMensaje con el valor de error como parámetro
                    mandaMensaje("Credenciales Inválidas ["+error.trim()+"]");
